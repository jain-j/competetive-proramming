	...
	while(scanf("%c",&command) != EOF) {
		...
		switch (command) {
			case 'I' : scanf("%d %d", &m, &n);
				...
				break;
			case 'C' : fillRectangle(1, 1, m, n, 'O', image);
				break;
			case 'L' :scanf("%d %d %c", &x1, &y1, &color);
				image[y1][x1] = color;
				break;
			case 'V' :scanf("%d %d %d %c", &x1, &y1, &y2, &color);
				if (y2 >= y1)
					fillRectangle(x1, y1, x1, y2, color, image);
				else
					fillRectangle(x1, y2, x1, y1, color, image);
				break;
			case 'H' :scanf("%d %d %d %c", &x1, &x2, &y1, &color);
				if (x2 >= x1)
					fillRectangle(x1, y1, x2, y1, color, image);
				else
					fillRectangle(x2, y1, x1, y1, color, image);
				break;
			case 'K' :scanf("%d %d %d %d %c", &x1, &y1, &x2, &y2, &color);
				if (x1 >= x2) {
					tmp = x1;
					x1 = x2;
					x2 = tmp;
